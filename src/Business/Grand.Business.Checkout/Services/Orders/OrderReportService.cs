...
public class OrderReportService : IOrderReportService
{
    ...
    /// <param name="endTimeUtc">End date</param>
    /// <returns>Result</returns>
    public virtual async Task<IList<OrderByTimeReportLine>> GetOrderByTimeReport(string storeId,
        DateTime? startTimeUtc = null,
        DateTime? endTimeUtc = null)
    {
        var report = new List<OrderByTimeReportLine>();
        ...
        string tagId = null)
    {
        ...
        if (!string.IsNullOrEmpty(billingCountryId))
            builderquery = builderquery.Where(o =>
                o.BillingAddress != null && o.BillingAddress.CountryId == billingCountryId);
    ...
    public virtual async Task<OrderAverageReportLineSummary> OrderAverageReport(string storeId, int os)
    {
        ...
        var nowDt = _dateTimeService.ConvertToUserTime(DateTime.Now);
        var timeZone = _dateTimeService.CurrentTimeZone;
        ...
        //all time
        var allTimeResult = await GetOrderAverageReportLine(storeId, os: os);
        ...
        bool showHidden = false)
    {
        ...
        var queryItemOrdered = orderBy == 1
            ? queryItem.OrderByDescending(x => x.TotalQuantity).ToList()
