...
public class MaintenanceController : BaseAdminController
{
    ...
    public async Task<IActionResult> MaintenanceDeleteGuests(MaintenanceModel model)
    {
        ...
        DateTime? endDateValue = model.DeleteGuests.EndDate == null
            ? null
            : _dateTimeService.ConvertToUtcTime(model.DeleteGuests.EndDate.Value, _dateTimeService.CurrentTimeZone)
                .AddDays(1);

        TempData["NumberOfDeletedCustomers"] = await _customerService.DeleteGuestCustomers(startDateValue, endDateValue,
        ...
        [FromServices] ILogger<MaintenanceController> logger)
    {
        ...
        if (storageSettings.PictureStoreInDb)
        {
            ...
            foreach (var picture in pictures)
                try
                {
                    var pictureConverted =
                        pictureService.ConvertPicture(picture.PictureBinary, mediaSettings.ImageQuality);
                    await pictureService.UpdatePicture(picture.Id, pictureConverted, "image/webp", picture.SeoFilename,
                        picture.AltAttribute, picture.TitleAttribute,
        ...
        foreach (var x in entityUrls)
        {
            ...
            switch (entityName)
            {
                ...
                case "knowledgebasecategory":
                    detailsUrl = Url.Action("EditCategory", "Knowledgebase",
                        new { id = x.EntityId, area = Constants.AreaAdmin });
                    break;
                case "knowledgebasearticle":
                    detailsUrl = Url.Action("EditArticle", "Knowledgebase",
                        new { id = x.EntityId, area = Constants.AreaAdmin });
                    break;
            }
    ...
    public async Task<IActionResult> CustomCss()
    {
        ...
        if (file != null) model.Content = await _mediaFileStore.ReadAllText(pathFile);
    ...
    public async Task<IActionResult> RobotsTxt(RobotsTxtModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            if (robotsTxt == null)
            {
                await _robotsTxtService.InsertRobotsTxt(new RobotsTxt {
                    Name = model.Name,
                    Text = model.Text,
                    StoreId = storeScope
                });
