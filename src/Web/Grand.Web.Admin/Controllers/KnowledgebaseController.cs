...
public class KnowledgebaseController : BaseAdminController
{
    ...
    {
        var (knowledgebaseArticleGridModels, totalCount) =
            await _knowledgebaseViewModelService.PrepareKnowledgebaseArticleGridModel(parentCategoryId, command.Page,
                command.PageSize);
    ...
    public async Task<IActionResult> CreateCategory(KnowledgebaseCategoryModel model, bool continueEditing)
    {
        if (ModelState.IsValid)
        {
            ...
            return continueEditing
                ? RedirectToAction("EditCategory", new { knowledgebaseCategory.Id })
    ...
    public async Task<IActionResult> EditCategory(KnowledgebaseCategoryModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            knowledgebaseCategory =
                await _knowledgebaseViewModelService.UpdateKnowledgebaseCategoryModel(knowledgebaseCategory, model);
            Success(_translationService.GetResource("Admin.Content.Knowledgebase.KnowledgebaseCategory.Updated"));
            return continueEditing
                ? RedirectToAction("EditCategory", new { id = knowledgebaseCategory.Id })
                : RedirectToAction("List");
        }
    ...
    public async Task<IActionResult> DeleteCategory(string id)
    {
        ...
        {
            Error(_translationService.GetResource(
                "Admin.Content.Knowledgebase.KnowledgebaseCategory.Cannotdeletewitharticles"));
    ...
    public async Task<IActionResult> CreateArticle(KnowledgebaseArticleModel model, bool continueEditing)
    {
        if (ModelState.IsValid)
        {
            ...
            return continueEditing
                ? RedirectToAction("EditArticle", new { knowledgebaseArticle.Id })
                : RedirectToAction("EditCategory", new { id = model.ParentCategoryId });
        }
    ...
    public async Task<IActionResult> EditArticle(KnowledgebaseArticleModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            knowledgebaseArticle =
                await _knowledgebaseViewModelService.UpdateKnowledgebaseArticleModel(knowledgebaseArticle, model);
    ...
    [PermissionAuthorizeAction(PermissionActionName.Edit)]
    [HttpPost]
    public async Task<IActionResult> RelatedArticlesAddPopupList(DataSourceRequest command,
        KnowledgebaseArticleModel.AddRelatedArticleModel model)
    {
        var articles =
    ...
    {
        var articles =
            await _knowledgebaseService.GetRelatedKnowledgebaseArticles(articleId, command.Page - 1, command.PageSize);
        var gridModel = new DataSourceResult {
