...
public class DiscountController : BaseAdminController
{
    ...
    public async Task<IActionResult> GetDiscountRequirementMetaInfo(string discountRequirementId, string discountId)
    {
        ...
        var discountPlugin =
            _discountProviderLoader.LoadDiscountProviderByRuleSystemName(discountRequirement
        ...
        DiscountModel.AddBrandToDiscountModel model, [FromServices] IBrandService brandService)
    {
        var brands = await brandService.GetAllBrands(model.SearchBrandName,
            _contextAccessor.WorkContext.CurrentCustomer.StaffStoreId, command.Page - 1, command.PageSize, true);
    ...
    {
        var vendors =
            await vendorService.GetAllVendors(model.SearchVendorName, command.Page - 1, command.PageSize, true);

        //search for emails
        if (!string.IsNullOrEmpty(model.SearchVendorEmail))
        {
            var tempVendors =
                vendors.Where(x => x.Email.ToLowerInvariant().Contains(model.SearchVendorEmail.Trim()));
    ...
    public async Task<IActionResult> VendorAddPopup(DiscountModel.AddVendorToDiscountModel model)
    {
        ...
        if (model.SelectedVendorIds != null) await _discountViewModelService.InsertVendorToDiscountModel(model);
